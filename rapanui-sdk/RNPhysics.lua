...
function RNPhysics.setTimeToSleep(value)
  RNPhysics.world:setTimeToSleep(value)
end

--- sets the linear sleep tolerance
function RNPhysics.setLinearSleepTolerance(value)
  RNPhysics.world:setLinearSleepTolerance(value)
end

--- sets the angular sleep tolerance
function RNPhysics.setAngularSleepTolerance(value)
  RNPhysics.world:setAngularSleepTolerance(value)
end
...
function RNPhysics.createBodyFromImage(image, ...)
  ...
  if (fixturesReceived ~= 0) then
    ...
    for i = firstFixtureArgument, arg.n, 1 do
      ...
      tempFixture.density = tempFixture.density or 1
      tempFixture.friction = tempFixture.friction or 0.3
      tempFixture.restitution = tempFixture.restitution or 0
      tempFixture.filter = tempFixture.filter or { categoryBits = nil, maskBits = nil, groupIndex = nil }
      tempFixture.sensor = tempFixture.sensor or false
      tempFixture.filter.categoryBits = tempFixture.filter.categoryBits or 1
      tempFixture.center = tempFixture.center or { x = 0, y = 0 }
      ...
      tempFixture.radius = tempFixture.radius or h / 2
      ...
      tempFixture.name = tempFixture.pe_fixture_id or image.name
  ...
  elseif (fixturesReceived == 0) then --else arg[1]~=nil
    ...
    --only one fixture in this RNBody)
    aRNBody.fixturelist[1] = tempFixture

    --gives RNFixture a name
    tempFixture.name = image.name or tempFixture.pe_fixture_id
  ...
  --translate body to sprite position
  aRNBody.x = xx
  aRNBody.y = yy
...
function RNPhysics.createJoint(type, ...)
  ...
  if (type == "mouse") then
    ...
    dampingRatio = dampingRatio or 0.2

    joint = RNPhysics.world:addMouseJoint(bodyA.body, bodyB.body, targetX, targetY, maxForce, frequencyHz, dampingRatio)
  end
